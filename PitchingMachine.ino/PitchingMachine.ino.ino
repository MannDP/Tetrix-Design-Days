...
{  
  ...
  InitializeInterrupt();
  interrupts();
  Serial.begin(9600); //115200 baud, 8 data bits, 1 stop bit, no parity, XON/XOFF flow control
  while (!Serial) {
...
void loop()
{
  ...
  while (Serial.available() > 0) {
      stringSpeed = Serial.readString();
      incomingByte = stringSpeed.toInt();  
      Serial.println(incomingByte, DEC); 
   
      
      if (incomingByte > -255 && incomingByte < 255) {
          md.setSpeed2((float(incomingByte))/255.0, (float(incomingByte))/255.0);  
      }
